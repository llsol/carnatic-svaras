...
# Define window size (e.g., 0.2 seconds) for segments before and after transition
...
def extract_features(segment, previous_or_next):
    ...
    features[f'mean_ber_{previous_or_next}'] = np.mean(ber)
...
def process_times(times, is_transition, is_not_transition):
    features = []
    for time_point in times:
        ...
        features_before['is_transition'] = is_transition
        features_before['is_not_transition'] = is_not_transition
        features_after['is_transition'] = is_transition
        features_after['is_not_transition'] = is_not_transition
...
feature_transitions.extend(process_times(transition_times, is_transition=1, is_not_transition=0))

# Process midpoints (non-transition points)
feature_transitions.extend(process_times(midpoints, is_transition=0, is_not_transition=1))

# Create a DataFrame from the combined feature differences
feature_transitions = pd.DataFrame(feature_transitions)

# Move 'is_transition' and 'is_not_transition' to the end of the DataFrame
columns = feature_transitions.columns.tolist()
columns.remove('is_transition')
columns.remove('is_not_transition')
columns.extend(['is_transition', 'is_not_transition'])
feature_transitions = feature_transitions[columns]
